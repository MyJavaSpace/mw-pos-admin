...
public class ExceptionResolverCustom implements HandlerExceptionResolver {
	...
	// json转换器
	// 将异常信息转json
	private HttpMessageConverter<ResultInfo> jsonMessageConverter;
	...
	// 异常信息解析方法
	private ResultInfo resolveExceptionCustom(Exception ex) {
	...
	private ModelAndView resolveJsonException(HttpServletRequest request, HttpServletResponse response, Object handler,
			Exception ex) {
		ResultInfo resultInfo  = resolveExceptionCustom(ex);
		resultInfo.setStackTrace(ex.getStackTrace());
		// 解析异常
//		ExceptionResultInfo exceptionResultInfo = resultInfo;
		HttpOutputMessage outputMessage = new ServletServerHttpResponse(response);

		try {
			// 将exceptionResultInfo对象转成json输出
			jsonMessageConverter.write(resultInfo, MediaType.APPLICATION_JSON, outputMessage);
	...
	public HttpMessageConverter<ResultInfo> getJsonMessageConverter() {
		return jsonMessageConverter;
	}

	public void setJsonMessageConverter(HttpMessageConverter<ResultInfo> jsonMessageConverter) {
